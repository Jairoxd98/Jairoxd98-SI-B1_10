...
public class Main {
    ...
    public static Stack<Nodo> busqueda (Estado inicial, Grid g, String estrategia, int cota, int []objetivo){
    	...
    	//boolean [][] visitados = new boolean[g.getRows()][g.getCols()];
    	...
    	/*try {
			pw = new PrintWriter(new File("ficheo.txt"));
    	...
    	while (!frontera.isEmpty() && !esObjetivo(frontera.peek(), objetivo)) { //no voy a tratar de buscar si ya he mirado en todo, o si ya he encontrado la solución
    		...
    		System.out.println("\tFRONTERA " + id);
    		while (!frontera.isEmpty()) {
    			System.out.println("\t" + frontera.peek());
    			fronteraAux.add(frontera.poll());
    		}
    		
    		while (!fronteraAux.isEmpty()) {
    			frontera.add(fronteraAux.poll());
    		}
    		...
    		//pw.println(nodo.toString() + " padre");
    		...
    		//visitados[nodo.getEstado().getId()[0]][nodo.getEstado().getId()[1]] = true;
...
private static ArrayList<Nodo> nodoSucesores (Nodo n, Grid g, String estrategia, int id, ArrayList<Nodo> visitados){
    	...
    	for (Estado a: funcionSucesores(n.getEstado(), g)) {
    		nAux = new Nodo(n, a, id, (n.getCosto()+a.getValor())+1, a.getAccion(), n.getD()+1, calcularHeuristica(a, g), n.getD()+1);
    		if(!visitados.contains(nAux)) {
    			++id;
    		
    			System.out.println(id+ "; " + n.getId()+" ("+a.getId()[0]+", "+a.getId()[1]+") "+ " "+ ((n.getCosto()+a.getValor())+1) +" "+ a.getAccion() +" "+ (n.getD()+1) +" "+ calcularHeuristica(a, g));
